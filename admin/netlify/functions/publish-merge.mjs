...
import { mergePublication, publicationErrorResponse, validateControlInput } from '../../src/server/publisher.mjs';
import { mutationResponse, requireJsonMutation } from '../../src/server/request-guards.mjs';

export default async function publishMerge(request) {
  const context = await requireJsonMutation(request, { maxBytes: 16 * 1024 });
  if (context.response) return context.response;
  try {
    const input = validateControlInput(context.body);
    const result = await mergePublication(input.number, input.headSha, { token: context.session.token });
